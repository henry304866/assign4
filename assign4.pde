...
void draw() {
  ...
  case GAME_START:
    ...
    printText("Press ENTER to Start",20,260); // replace this with printText
    /*--------------------------------*/
    break;
  ...
  case GAME_PAUSE:
    /*---------Print Text-------------*/
     ...
     printText("Press ENTER to Resume",20,260);
    /*--------------------------------*/
    break;
...
void alienMaker() {
  for(int num = 0 ; num < 54 ; num++){
    if(num>0&&num%12==0){
     nums++;}
  aList[num]= new Alien(50 + (num%12)* 40  ,50 + nums * 50 );}
}
